  ...
  useEffect(() => {
    ...
    async function loadAndSelect() {
      ...
      try {
        ...
        // 1. Parallel execution of independent API and Stream Chat queries
        const [netRes, queriedChannelsRes, groupRes, removedRes] = await Promise.allSettled([
          userService.getNetworkRequests(),
          ...
          chatService.getRemovedGroups(),
        ]);

        if (!isMounted) return;

        // Process accepted connections
        const acceptedConnIdsSet = new Set();
        if (netRes.status === "fulfilled" && netRes.value && Array.isArray(netRes.value.connections)) {
          netRes.value.connections.forEach((conn) => {
            const partnerId = conn.userId || conn.id || conn._id;
            if (partnerId) acceptedConnIdsSet.add(String(partnerId));
          });
        }

        const queriedChannels = queriedChannelsRes.status === "fulfilled" && Array.isArray(queriedChannelsRes.value) ? queriedChannelsRes.value : [];
        const dbGroups = groupRes.status === "fulfilled" && Array.isArray(groupRes.value?.groups) ? groupRes.value.groups : [];
        const removedDbGroups = removedRes.status === "fulfilled" && Array.isArray(removedRes.value?.groups) ? removedRes.value.groups : [];

        // 2. Parallel watching of group channels
        const dbGroupPromises = dbGroups
          .filter((g) => g.streamChannelId)
          .map(async (g) => {
            try {
              ...
              console.error("Error watching persistent group channel:", g.streamChannelId, err);
              return null;
            }
          });

        const removedGroupPromises = removedDbGroups
          .filter((rg) => rg.streamChannelId && !dbGroups.some((g) => g.streamChannelId === rg.streamChannelId))
          .map(async (rg) => {
            try {
        ...
        const [dbGroupChannelsRaw, removedGroupChannelsRaw] = await Promise.all([
          Promise.all(dbGroupPromises),
          Promise.all(removedGroupPromises),
        ]);

        const dbGroupChannels = dbGroupChannelsRaw.filter(Boolean);
        const removedGroupChannels = removedGroupChannelsRaw.filter(Boolean);

        // 3. Direct Message Target Channel Resolution
        if (
          targetUserIdParam &&
          ...
          if (acceptedConnIdsSet.size > 0 && !acceptedConnIdsSet.has(targetId)) {
            ...
            if (existing) {
              ...
              await targetChannel.watch().catch(() => {});
            } else {
              try {
                ...
                showToast(err.message || "You are no longer connected with this user.");
                if (isMounted) {
                  setActiveChannel(null);
                  setMobileShowChat(false);
                  navigate("/messages", { replace: true });
            ...
            if (targetChannel && isMounted) {
              try {
                const members = Object.values(targetChannel.state?.members || {});
                const otherMember = members.find(
                  (m) => String(m.user_id || m.user?.id) !== String(currentUserId)
                );
                const otherUser = otherMember?.user || {};
                if (!otherUser.name || otherUser.name === targetId) {
                  const profileRes = await userService.getParticipantProfile(targetId);
                  if (profileRes?.user) {
                    const u = profileRes.user;
                    targetChannel.data = {
                      ...(targetChannel.data || {}),
                      targetName: u.name,
                      targetAvatar: u.avatar || u.profile?.avatar || "",
                    };
                  }
                }
              } catch (profileError) {
                console.warn("Could not load target getHack profile:", profileError);
              }
            }
          }
        }
        ...
        // 4. Build final left-side chat list
        const combinedList = [];
        ...
        console.error("FAILED TO LOAD/SELECT CHAT:", error);
        if (isMounted) {
          setChannelError(error.message || "Failed to load conversations.");
        }
      } finally {
        if (isMounted) {
